...
const formatServiceDetails = (serviceType: 'main' | 'transport', details: { durationHours?: string; numberOfTrips?: string }) => {
  if (serviceType === 'main') {
...
export const ServerRoute = createServerFileRoute('/api/wedding-booking').methods({
  POST: async ({ request }) => {
    try {
      ...
      const vehicleLabel = selectedVehicleName || selectedVehicle || 'Not specified'
      const decorationLabel = decorationOptionName || decorationOption || 'None'
      const formattedDecorationPrice = decorationPrice ? `€${decorationPrice}` : 'N/A'
      const formattedTotalPrice = formatCurrency(typeof calculatedPrice === 'number' ? calculatedPrice : null)
      const serviceLabel = serviceType === 'transport' ? 'Guest Transport' : 'Main Wedding Fleet'
      ...
      return new Response(JSON.stringify({ message: 'Wedding booking submitted successfully' }), { status: 200 })
    } catch (error) {
      console.error('Wedding booking submission failed:', error)
      return new Response(JSON.stringify({ error: 'Unable to process booking at this time.' }), { status: 500 })
    }
  }
})
