...
export const ServerRoute = createServerFileRoute('/api/tour-booking').methods({
  POST: async ({ request }) => {
    try {
      ...
      const formattedTotalPrice = formatCurrency(typeof totalPrice === 'number' ? totalPrice : null)
      const formattedVehiclePrice = formatCurrency(typeof vehicleMinPrice === 'number' ? vehicleMinPrice : null)
      const distanceLabel = calculatedDistanceKm ? `${calculatedDistanceKm.toFixed(1)} km` : 'N/A'
      const addOnsList = (addOnDetails as string[] | undefined)?.filter(Boolean)
      const addOnsHtml = addOnsList && addOnsList.length > 0
      ...
      return new Response(JSON.stringify({ message: 'Tour booking submitted successfully' }), { status: 200 })
    } catch (error) {
